...
public class ExceptionUtils {
    private String duplicateMatcher(String errorMessage) {
        final String DUPLICATION_PATTERN = "Key \\(([^)]+)\\)=\\(([^)]+)\\) already exists";
        Pattern pattern = Pattern.compile(DUPLICATION_PATTERN);
    ...
    private String entityNotFoundMatcher(String errorMessage) {
        final String ENTITY_NOT_FOUND_PATTERN = "Unable to find (\\S+) with (\\S+)";
        Pattern pattern = Pattern.compile(ENTITY_NOT_FOUND_PATTERN);
    ...
    public String getJpaObjectRetrievalFailureExceptionMessage(String errorMessage) {
        String entityNotFoundMessage = entityNotFoundMatcher(errorMessage);
        if (entityNotFoundMessage != null) return entityNotFoundMessage;
        return "Invalid data";
    }
}